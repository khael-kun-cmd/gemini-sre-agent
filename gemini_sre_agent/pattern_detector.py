...
# ============================================================================
# Layer 4: Advanced Confidence Scoring System
# ============================================================================
...
class ConfidenceFactors:
    ...
    # Temporal factors
    TIME_CONCENTRATION = "time_concentration"  # How concentrated errors are in time
    TIME_CORRELATION = "time_correlation"     # Temporal correlation between errors
    RAPID_ONSET = "rapid_onset"               # Errors occur within short time
    GRADUAL_ONSET = "gradual_onset"           # Errors build up gradually
    
    # Service impact factors
    SERVICE_COUNT = "service_count"           # Number of affected services
    SERVICE_DISTRIBUTION = "service_distribution"  # How errors spread across services
    CROSS_SERVICE_CORRELATION = "cross_service_correlation"  # Inter-service error correlation
    
    # Error pattern factors
    ERROR_FREQUENCY = "error_frequency"       # Rate of errors
    ERROR_SEVERITY = "error_severity"         # Severity distribution
    ERROR_TYPE_CONSISTENCY = "error_type_consistency"  # Similar error types
    MESSAGE_SIMILARITY = "message_similarity"  # Similar error messages
    
    # Historical factors
    BASELINE_DEVIATION = "baseline_deviation"  # Deviation from historical baseline
    TREND_ANALYSIS = "trend_analysis"         # Error trend over time
    SEASONAL_PATTERN = "seasonal_pattern"     # Known patterns at this time
    
    # External factors
    DEPENDENCY_STATUS = "dependency_status"   # Status of external dependencies
    RESOURCE_UTILIZATION = "resource_utilization"  # System resource usage
    DEPLOYMENT_CORRELATION = "deployment_correlation"  # Recent deployments
...
class ConfidenceRule:
    ...
    threshold: Optional[float] = None  # Minimum value to contribute
    max_contribution: float = 1.0  # Maximum contribution to overall score
    decay_function: Optional[str] = None  # linear, exponential, logarithmic
...
class ConfidenceScore:
    ...
    raw_factors: Dict[str, float]  # Raw factor values before weighting
    confidence_level: str  # VERY_LOW, LOW, MEDIUM, HIGH, VERY_HIGH
    explanation: List[str]  # Human-readable explanation of score


class ConfidenceScorer:
    """Advanced confidence scoring engine for pattern detection."""
    
    def __init__(self, confidence_rules: Optional[Dict[str, List[ConfidenceRule]]] = None):
        """
        Initialize the confidence scorer.
        
        Args:
            confidence_rules: Custom confidence rules per pattern type
        """
    ...
    def calculate_confidence(
        ...
        """
        Calculate comprehensive confidence score for a pattern detection.
        
        Args:
            pattern_type: Type of pattern being evaluated
            window: Time window containing the logs
            threshold_results: Results from threshold evaluation
            logs: Log entries for the pattern
        ...
        raw_factors = self._calculate_raw_factors(window, threshold_results, logs, context)
        
        # Apply rules and weights to get factor scores
        factor_scores = {}
        weighted_sum = 0.0
        total_weight = 0.0
    ...
    def _calculate_raw_factors(
        ...
        """Calculate raw values for all confidence factors."""
        factors = {}
        
        # Temporal factors
        factors[ConfidenceFactors.TIME_CONCENTRATION] = self._calculate_time_concentration(logs, window)
        ...
        # Service impact factors
        services = list(set(log.service_name for log in logs if log.service_name))
        ...
        factors[ConfidenceFactors.CROSS_SERVICE_CORRELATION] = self._calculate_cross_service_correlation(logs)
        ...
        # Historical factors (simplified for now)
        ...
        # External factors (simplified for now)
    ...
    def _calculate_time_correlation(self, logs: List[LogEntry]) -> float:
        ...
        # High correlation if errors happen within 2 minutes
    ...
    def _calculate_service_distribution(self, logs: List[LogEntry]) -> float:
        ...
        if len(service_counts) <= 1:
            return 0.0  # All errors from one service
    ...
    def _calculate_cross_service_correlation(self, logs: List[LogEntry]) -> float:
        ...
        # Calculate time overlap between services
        ...
        for i in range(len(services)):
            for j in range(i + 1, len(services)):
                service_a_times = service_timestamps[services[i]]
                service_b_times = service_timestamps[services[j]]
                
                # Simple correlation: check if errors happen within 30 seconds of each other
                correlation = 0.0
                for time_a in service_a_times:
                    for time_b in service_b_times:
                        if abs((time_a - time_b).total_seconds()) <= 30:
                            correlation += 1
                
                if service_a_times and service_b_times:
                    normalized_correlation = correlation / (len(service_a_times) * len(service_b_times))
                    correlation_sum += normalized_correlation
    ...
    def _calculate_severity_factor(self, logs: List[LogEntry]) -> float:
        """Calculate confidence factor based on error severity distribution."""
        if not logs:
            return 0.0
    ...
    def _calculate_error_consistency(self, logs: List[LogEntry]) -> float:
        """Calculate how consistent error types are across logs."""
        if not logs:
            return 0.0
        
        # Simple consistency check based on severity
    ...
    def _calculate_message_similarity(self, logs: List[LogEntry]) -> float:
        """Calculate similarity of error messages."""
        if not logs:
            return 0.0
        ...
        # Calculate Jaccard similarity between messages
        similarities = []
    ...
    def _check_gradual_onset(self, logs: List[LogEntry]) -> bool:
        """Check if errors show gradual onset pattern."""
        if len(logs) < 3:
            return False
        ...
        # Check if error frequency increases over time
        # Split into three time buckets and check if later buckets have more errors
        total_time = (sorted_logs[-1].timestamp - sorted_logs[0].timestamp).total_seconds()
        if total_time < 60:  # Less than 1 minute - not gradual
            return False
    ...
    def _determine_confidence_level(self, score: float) -> str:
        """Determine confidence level category from numeric score."""
        if score >= 0.9:
            return "VERY_HIGH"
        elif score >= 0.75:
            return "HIGH"
    ...
    def _get_default_confidence_rules(self) -> Dict[str, List[ConfidenceRule]]:
        """Get default confidence rules for each pattern type."""
        return {
            ...
            PatternType.SPORADIC_ERRORS: [
                ...
                ConfidenceRule(ConfidenceFactors.ERROR_TYPE_CONSISTENCY, 0.2, decay_function="linear"),
...
class PatternClassifier:
    ...
    def _detect_cascade_failure(
        ...
        if cascade_triggers or (service_impact_triggers and 
            ...
            # Calculate confidence using the advanced scoring system
    ...
    def _detect_traffic_spike(
        ...
        if frequency_triggers:
            ...
            confidence_score = self.confidence_scorer.calculate_confidence(
                ...
                window=window,
                threshold_results=frequency_triggers,
                logs=all_logs,
                additional_context={
    ...
    def _detect_sporadic_errors(
        ...
        if service_distribution > 0.3 and time_distribution > 0.4:
            confidence_score = self.confidence_scorer.calculate_confidence(
                ...
                window=window,
                threshold_results=triggered_results,
                logs=all_logs,
                additional_context={
